...
```{r}
grouped_data <- group_by(df, gender) 
summary_bb_bygender <- summarise(grouped_data, mean(balloon_balance))
...
```{r}
grouped_data_cola <- group_by(df, taste_cola) 
summary_slp_bycola <- summarise(grouped_data_cola, mean(sound_level_pref))
...
```{r}
grouped_data_hand <- group_by(df, handedness)
summary_tt_byhand <- summarise(grouped_data_hand, mean(tongue_twist))
...
summary_tt_byhand_number <- summarise(n_hand = n(),grouped_data_hand, mean(tongue_twist))




summary_tt_byhand_number
```
  
  
  3b. What does number of people tell us about the estimates of the mean? Are our results reliable?
```{r}
#the mean toungetwisting time for right-handed partisipants might be more representative than for left-handed people and ambidextrous people here only represented by 1 person is not representative at all
```
...
```{r}
ggplot(df, aes(x=tongue_twist))+geom_histogram(binwidth = 1) 

#you can change binwidth if you want to, it's cosmetic
...
```{r}
ggplot(df, aes(x=gender, y=balloon))+geom_bar(stat='summary', fun.y = mean) 

#we ask to show us the mean of values on y coordinate
```
...
ggplot(df, aes(x=handedness, y=shoesize, fill = handedness))+
  geom_bar(stat='summary', fun.y = mean, width = 0.5)+
  geom_errorbar(stat = 'summary', fun.data = mean_se, width = 0.2)
  labs(x = "Prefered hand", y = "Shoesize")+
  theme_minimal()
